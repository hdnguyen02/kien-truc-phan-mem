...
const Classes = () => {
  ...
  async function getDecks(searchDecks) {
    let subUrl;
    if (searchDecks && searchDecks != "")
      subUrl = `/decks?searchTerm=${searchDecks}`;
    else subUrl = `/decks`;
    try {
              ...
              <div className="p-4 md:p-5 space-y-4">
                <div>
                  <label className="block mb-2 text-sm font-medium text-gray-900">
                    Tên bộ thẻ <span className="text-ctred">*</span>
                  </label>
                  <input
                    onChange={handleChangeNameClassCreate}
                    type="text"
                    required
                ...
                <div>
                  ...
                  <input
                    onChange={handleChangeDecsClassCreate}
                    type="text"
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                  />
                </div>
